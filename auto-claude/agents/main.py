...
Prerequisites:
  1. Create a spec first: claude /spec (or gemini /spec)
  2. Set your API key:
     - For Claude: run 'claude setup-token' and set CLAUDE_CODE_OAUTH_TOKEN
     - For Gemini: set GEMINI_API_KEY (and optional: AUTO_CLAUDE_CLI_COMMAND=gemini)

Environment Variables:
  CLAUDE_CODE_OAUTH_TOKEN  Your Claude Code OAuth token
  GEMINI_API_KEY           Your Gemini API key
  AUTO_CLAUDE_CLI_COMMAND  CLI command to use (default: claude)
