...
Assistant:"""
    ...
    try:
        # Try to use CLI with --print for simple output
        cli_command = os.environ.get("AUTO_CLAUDE_CLI_COMMAND", "claude")
        cmd = [cli_command]
        
        if cli_command == "claude":
            cmd.extend(["--print", "-p", full_prompt])
        else:
            # gemini prefers positional argument ( -p is deprecated)
